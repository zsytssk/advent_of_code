...
fn parse1() {
  ...
  let map = parse_input();
  let mut elves = map.get_elf_points();
  ...
  'outer: loop {
    ...
    for (item_index, item) in elves.iter().enumerate() {
      for num in 0..4 {
        ...
        let not_move = not_need_move(item, &elves);
        if not_move {
          break;
        }
        let is_move = can_move(item, cur_dir, &elves);
        if is_move {
...
fn parse_input() -> Map {
  let content = read_file("day23/demo1.txt").unwrap();
  ...
  let range = get_range(&points.iter().map(|item| item.borrow_mut()).collect());
